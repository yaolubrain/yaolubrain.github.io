...
function init() {
    ...
    for (var i = 0; i < data.length; ++i) {
        if (data[i][4] > 2.5) {
            var label = makeTextSprite( classNames[i], data[i][4], { r:0, g:0, b:0, a:1.0 } );        
...
function onKeyDown( event ) {      
    if (event.keyCode == "80") {
        ...
        saveFile(imgData.replace(strMime, strDownloadMime), "nips.jpg");
