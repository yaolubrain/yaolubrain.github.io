...
function init() {
    ...
    for (var i = 0; i < 20; ++i) {
         var color = new THREE.MeshPhongMaterial( { color: (i / 20 + 0.01) * 0xffff00 } );   
         pointMaterial.push(color);
    }

    for (var i = 0; i < data.length; ++i) {
        var pointMesh = new THREE.Mesh(pointGeo, pointMaterial[data[i][3]-1]);           
        pointMesh.position.set( 0, 0, 1.0001 );
        ...
        point.rotateOnAxis( rotAxis, - pos.angleTo(zAxis) );
...
function onKeyDown( event ) {      
    if (event.keyCode == "80") {
        ...
        saveFile(imgData.replace(strMime, strDownloadMime), "coil20.jpg");
